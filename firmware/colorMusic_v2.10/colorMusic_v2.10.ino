...
/*
 *  1) REMOTE_TYPE : 1 (WAVGAT) --> 2 (KEYES)
 *
 *  2) KEEP_SETTINGS : 1 --> 0
 *  3) KEEP_STATE : 1 --> 0
 *  4) RESET_SETTINGS : 0 --> 1 --> 0
 *
 *  5) NUM_LEDS : 60 --> 288
 *  6) BRIGHTNESS : 200 --> 10
 *
 *  7) MODE : 0 --> 4 (freq_strobe_mode)
 *
 *  8) AUTO_LOW_PASS : 0 --> 1
 *  9) EEPROM_LOW_PASS : 1 --> 0
 *
 * 10) Blinking "L" LED (see below)
 *
 */
...
// ----- настройки ленты
#define NUM_LEDS 288        // количество светодиодов (данная версия поддерживает до 410 штук)
...
/* Blinking "L" LED - start 1 */
#define COUNTDOWN_VALUE 27
int countdown = COUNTDOWN_VALUE;
int led_state = 0;
/* Blinking "L" LED - end 1 */
